...
// 构建组件属性
function buildAttributes(scheme: { [x: string]: any; __config__: any; __slot__: any; options: any; __vModel__: any; props: { props: any }; action: any }, dataList: any[], ruleList: any[], optionsList: any[], methodList: any[], propsList: any[], uploadVarList: any[], created: any[]) {
...
function buildBeforeUpload(scheme: { __config__: any; accept: any; __vModel__: any }) {
  const config = scheme.__config__
  const unitNum = (units as any)[config.sizeUnit]; let rightSizeCode = ''; let acceptCode = ''; const
    returnList = []
  if (config.fileSize) {
